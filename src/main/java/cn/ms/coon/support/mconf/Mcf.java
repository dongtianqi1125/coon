...
public class Mcf {
	...
	// build app
	public Mcf buildApp(String node, String app, String... pairs) {
		return this.buildApp(node, app, NURL.toStringMap(pairs));
	}

	public Mcf buildApp(String node, String app, Map<String, String> appAttrs) {
		if (AbstractMconf.isNotBlank(node)) {
			this.setNode(node);
		}

		if (AbstractMconf.isNotBlank(app)) {
			this.setApp(app);
		}
	...
	// build conf
	public Mcf buildConf(String env, String group, String version, String conf,String... pairs) {
		return this.buildConf(env, group, version, conf, NURL.toStringMap(pairs));
	}

	public Mcf buildConf(String env, String group, String version, String conf,
			Map<String, String> confAttrs) {
		if (AbstractMconf.isNotBlank(env)) {
			this.setEnv(env);
		}
		if (AbstractMconf.isNotBlank(group)) {
			this.setGroup(group);
		}
		if (AbstractMconf.isNotBlank(version)) {
			this.setVersion(version);
		}
		if (AbstractMconf.isNotBlank(conf)) {
			this.setConf(conf);
		}
		if (confAttrs != null) {
	...
	// build data
	public Mcf buildData(String data, String... pairs) {
		return this.buildData(data, NURL.toStringMap(pairs));
	}

	public Mcf buildData(String data, Map<String, String> dataAttrs) {
		if (AbstractMconf.isNotBlank(data)) {
			this.setData(data);
		}
		if (dataAttrs != null) {
	...
	public String getPrefixKey() {
		if (AbstractMconf.isBlank(root)) {// check root
			throw new IllegalArgumentException("The must set 'root'.");
		}
		if (AbstractMconf.isBlank(app)) {// check app
			throw new IllegalArgumentException("The must set 'app'.");
		}
		if (AbstractMconf.isBlank(conf)) {// check conf
	...
	public String getSuffixKey() {
		if (AbstractMconf.isBlank(data)) {// check data
	...
	public static String buildAttributes(Map<String, String> attributes) {
		...
		try {
			...
			for (Map.Entry<String, String> item : infoIds) {
				if (AbstractMconf.isNotBlank(item.getKey())) {
